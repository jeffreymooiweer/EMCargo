...
export default function DgReviewsPage({ user }: { user: User }) {
  ...
  return <div className="collection-page page-enter dg-review-workspace">
    ...
    {loading ? <p role="status">{t("wizard.loading")}</p> : !failure && !id ? <section className="surface">
      ...
      {shipment.routing && <ShipmentRoutingSummary routing={shipment.routing} lines={shipment.lines} />}
      ...
      <footer className="review-footer">{detail.created_by === user.username && <Link className="action-secondary" to={`/wizard/${shipment.modality || "preparation"}?review=${detail.id}`}>{t("dgReview.openWizard")}<ArrowRightIcon /></Link>}
