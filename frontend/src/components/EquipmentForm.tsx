...
export default function EquipmentForm({ initial, onSave, onClose, busy }: {
  ...
  return <form onSubmit={event => { event.preventDefault(); void onSave(form); }} className="space-y-6">
    <fieldset disabled={busy} className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {form.kind === "container" && <>{text("container_number")}{text("container_type")}{text("inspection_due", "inspection_due", { date: true })}</>}
      </div>
      ...
      {form.kind !== "container" && <section><h4 className="mb-3 font-semibold">{t("assets.configurations")}</h4>
